...
export const getAll = async () => {
    try {
        const result = await PgSingleton.find(`SELECT d.* FROM diseases d WHERE d.status = ${EStatus.ACTIVE} ORDER BY d.name`);
        if(!result)
...
export const update = async (disease) => {
    try {
        const diseases = await PgSingleton.findOne(`SELECT d.* FROM diseases d WHERE d.name = '${disease.name}'`);
        if(diseases)
